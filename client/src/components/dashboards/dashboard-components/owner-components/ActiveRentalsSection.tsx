...
const ActiveRentalsSection: React.FC = () => {
  ...
  if (!activeRentals || activeRentals.length === 0) {
    return (
      <div className="p-6 sm:p-8 text-center bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
        <FaCar className="mx-auto text-3xl sm:text-4xl text-gray-400 mb-3" />
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <button
          onClick={() => refetch()}
          className="px-3 py-1.5 text-xs sm:text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors w-full sm:w-auto"
        >
          Refresh
        </button>
      </div>
        ...
        {activeRentals.map((rental: Rental) => (
          ...
          >
            ...
            <div className="relative h-32 sm:h-40 bg-gray-200 dark:bg-gray-700">
              {rental.car?.images?.[0]?.imageUrl ? (
                ...
                <div className="flex items-center justify-center h-full">
                  <FaCar className="text-3xl sm:text-4xl text-gray-400" />
                </div>
              )}
              
              {/* Status Badge */}
              <div className="absolute top-2 right-2">
                <span className="px-2 py-0.5 sm:py-1 bg-blue-600 text-white text-xs font-medium rounded-full">
            ...
            <div className="p-3 sm:p-4">
              ...
              {/* Customer Info */}
              <div className="mb-2 sm:mb-3 flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-300">
                <FaUser className="text-gray-400 flex-shrink-0" />
                <span className="truncate">{rental.customer?.name}</span>
              </div>

              {/* Rental Amount */}
              <div className="mb-2 sm:mb-3 p-2 bg-gray-50 dark:bg-gray-700 rounded">
