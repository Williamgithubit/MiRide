 ...
 */
export const exportUserReportToPDF = (reportData: any) => {
  const doc = new jsPDF();
  ...
  if (reportData.usersByRole && reportData.usersByRole.length > 0) {
    ...
    doc.autoTable({
      head: [['Role', 'Count']],
      body: reportData.usersByRole.map((item: any) => [item.role, item.count]),
      startY: 95,
      styles: { fontSize: 10 },
 ...
 */
export const exportRevenueReportToPDF = (reportData: any) => {
  const doc = new jsPDF();
  ...
  if (reportData.revenueByMonth && reportData.revenueByMonth.length > 0) {
    ...
    doc.autoTable({
      ...
      body: reportData.revenueByMonth.map((item: any) => [
        item.month,
